...
def test_stakeholder_analysis():
    print("\nTesting Stakeholder Analysis Endpoint")
    ...
    report_payload = {
        "request_id": f"test-stakeholder-analysis-{uuid.uuid4()}",
    ...
    start_response = requests.post(
        f"{BASE_URL}/api/stakeholder_analysis/analyze-stakeholders",
        json=report_payload,
        headers=headers,
        timeout=60
    )
    ...
    status = wait_for_task_completion(
        base_url=BASE_URL,
        status_path="/api/stakeholder_analysis/analyze-stakeholders/status",
        request_id=report_payload["request_id"],
        headers=headers
    )

    if status.get("status") == "completed":
        print("Stakeholder analysis completed")
    else:
        print(f"Stakeholder analysis failed: {status.get('error')}")

    result_response = requests.get(
        f"{BASE_URL}/api/stakeholder_analysis/analyze-stakeholders/result",
    ...
    fixes_payload = {
        "request_id": f"test-stakeholder-fixes-{uuid.uuid4()}",
        "bill_text": inspected_user_text,
        "phase": "fixes",
        "report_context": {
    ...
    with open("outputs/step9_stakeholders.json", "w") as file:
        json.dump(result, file, indent=2)

    print("Output saved to: outputs/step9_stakeholders.json")
    return result

if __name__ == "__main__":
    test_stakeholder_analysis()