...
class Settings:
    ...
    codex_originator: str = os.getenv("CODEX_ORIGINATOR", "step1-similarity")
    ...
    llm_rerank_input_limit: int = _int_env("LLM_RERANK_INPUT_LIMIT", 30)
    final_result_limit: int = _int_env("FINAL_RESULT_LIMIT", 12)
    excerpt_char_limit: int = _int_env("EXCERPT_CHAR_LIMIT", 1200)
    raw_text_char_limit: int = _int_env("RAW_TEXT_CHAR_LIMIT", 12000)

    embedding_model: str = os.getenv("EMBEDDING_MODEL", "sentence-transformers/all-MiniLM-L6-v2")
    embedding_device: str = os.getenv("EMBEDDING_DEVICE", "cpu")
    embedding_batch_size: int = _int_env("EMBEDDING_BATCH_SIZE", 16)

    require_bootstrap_indexes: bool = _bool_env("REQUIRE_BOOTSTRAP_INDEXES", False)
